<template>
    <DashboardLayout>
        <div class="flex items-center sm:flex-col md:flex-row  lg:justify-between border-bot-only py-2 border-bot-only   mb-4">
            <span class="text-[20px] font-bold text-gray-500">Departments Page</span> 
            <div class="relative">
                <input v-model="searchField"  type="text" placeholder="seach" class="rounded-md ">
        ...
        <div>
            <div class="relative overflow-x-auto shadow-md sm:rounded-lg ">
                <table class="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400">
                    ...
                    <tbody>
                        <tr v-for="dep in filteredData" :key="dep.id" class="odd:bg-white odd:dark:bg-gray-900 even:bg-gray-50 even:dark:bg-gray-800 border-b dark:border-gray-700">
                            ...
                            <td  v-if="!dep.department_heads.length"  class="px-6 py-4">
                                <span> -- </span>
                            </td>
        ...
        <div class="flex justify-center items-center mt-4">
            <button @click="prevPage" :disabled="currentPage === 1" class="px-4 py-2 mx-1 bg-blue-600 text-white rounded-md disabled:opacity-50">Previous</button>
            <span class="px-4 py-2">{{ currentPage }} / {{ totalPages }}</span>
            <button @click="nextPage" :disabled="currentPage === totalPages" class="px-4 py-2 mx-1 bg-blue-600 text-white rounded-md disabled:opacity-50">Next</button>
        </div>
        <!--PAGINATION CONTROLS-->
    </DashboardLayout>
